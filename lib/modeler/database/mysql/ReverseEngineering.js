...
class MySQLReverseEngineering {
  constructor(context, logger, connector) {
    ...
    this.reverseRules = { ...defaultRules,
      ...this.connector.options.reverseRules
    };
    this.saveIntermediate = context.saveIntermediate ?? false;
  }

  async reverse_(outputDir) {
    this.logger.log("info", `Pulling schema from ${this.connector.driver} database "${this.connector.database}" ...`);
  ...
  async extractTable_(entityName, table, targetGemlPath) {
    ...
    if (this.saveIntermediate) {
      fs.writeFileSync(path.join(targetGemlPath, table.TABLE_NAME + ".meta.json"), JSON.stringify(columns, null, 2));
    }
    ...
    if (this.saveIntermediate && !_.isEmpty(indexInfo)) {
      fs.writeFileSync(path.join(targetGemlPath, table.TABLE_NAME + ".index.json"), JSON.stringify(indexInfo, null, 2));
    }
    ...
    if (this.saveIntermediate && !_.isEmpty(referencesInfo)) {
      fs.writeFileSync(path.join(targetGemlPath, table.TABLE_NAME + ".ref.json"), JSON.stringify(referencesInfo, null, 2));
    }
  ...
  _refineEntityRelationships(mapOfEntities) {
    ...
    _.forOwn(mapOfEntities, ({
      ...
      entityInfo.associations.forEach(({
        ...
        if (type === "hasMany") {
          if (!backRef) {
            pushIntoBucket(entityAssoc, name, {
              type: "refersTo",
              srcField,
              destEntity
            });
          ...
          if (!backRef) {
            pushIntoBucket(entityAssoc, entity, {
              type: "hasMany",
              destEntity: name
            });
            return;
  ...
  _makeEntityManyToMany(entityName1, entityName2, entityAssoc) {
    pushIntoBucket(entityAssoc, entityName1, {
      type: "hasMany",
      destEntity: entityName2
    });
    pushIntoBucket(entityAssoc, entityName2, {
      type: "hasMany",
...
//# sourceMappingURL=ReverseEngineering.js.map